...
const fetchContactCard = async () => {
  ...
  if (existingCard && Object.keys(existingCard).length > 0) {
    // Establecer los valores del perfil existente
    contactCard.value.nutritionist_id = nutritionist.value.id;
    contactCard.value.slogan = existingCard.slogan;
    contactCard.value.address = existingCard.address;
